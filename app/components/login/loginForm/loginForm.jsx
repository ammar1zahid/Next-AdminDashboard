...
export default function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.ok) {
        console.log(
          "🎉 [LoginForm] Login successful! Verifying session before redirect"
        );
        ...
        const verifySessionAndRedirect = async (attempt = 1) => {
          ...
          try {
            ...
            console.log(
              "👤 [LoginForm] Session check result:",
              session ? "EXISTS" : "NULL"
            );
            ...
            if (session?.user) {
              console.log(
                "✅ [LoginForm] Session verified! Trying multiple redirect methods"
              );
            ...
            if (attempt < 5) {
              console.log(
                `⏰ [LoginForm] No session yet, retrying in ${attempt}00ms (attempt ${
                  attempt + 1
                })`
              );
              setTimeout(
                () => verifySessionAndRedirect(attempt + 1),
                attempt * 1000
              );
            } else {
              console.error(
                "❌ [LoginForm] Session verification failed after 5 attempts"
              );
              setError(
                "Login successful but session not established. Please refresh the page."
              );
            }
          } catch (sessionError) {
            ...
            if (attempt < 3) {
              ...
              console.log(
                "🔄 [LoginForm] Session check failed, attempting direct redirect"
              );
              window.location.href = "/dashboard";
            }
          }
        ...
        console.warn(
          "⚠️ [LoginForm] Login response neither ok nor error:",
          res
        );
      }
    } catch (err) {
